...
(define (insert val heap)
  (insTree (make-Node 0 (make-Elem val (unbox comparer)) null) heap))
...
(define (findMin heap)
  (cond
    ...
    [else (let ([x (root (car heap))]
                [y (findMin (cdr heap))])
            (if ((Elem-comparer (Node-elem (car heap))) x y) x y))]))
                        ...
                        (lambda (intheap)
                          (if (null? (cdr intheap))
                              ...
                              (let ([pair (getMin (cdr intheap))])
                                (if ((Elem-comparer (Node-elem (car pair))) 
                                     (root (car intheap)) 
                                     (root (car pair)))
                                    intheap
