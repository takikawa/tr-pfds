...
(check-expect (sorted-list (delete-min/max 
                            (bootstrapped-heap less-than? 1 2 -3 4)))
              (list 1 2 4))
(check-error (delete-min/max (delete-min/max (bootstrapped-heap less-than? 1))) 
...
(check-expect (sorted-list (insert 10 (bootstrapped-heap less-than? 1 -2 -3 -4)))
              (list -4 -3 -2 1 10))

(check-expect (sorted-list (insert -10 (bootstrapped-heap less-than? 1 20 -3 -4)))
              (list -10 -4 -3 1 20))
...
(check-expect (sorted-list (merge (delete-min/max (bootstrapped-heap less-than? -1))
                                  (bootstrapped-heap less-than? 1 -2 -3 -4)))
              (list -4 -3 -2 1))
