...
The function @scheme[bootstrapped-heap] creates a Bootstrapped Heap with the given 
inputs. For example,
@schememod[
typed-scheme
...
In the above example, the bootstrapped heap obtained will have elements 1 thru' 6 
...
@subsection{empty?}
The function @scheme[empty?] checks if the given bootstrapped heap is empty or not.
For example,
@schememod[
typed-scheme
(require "bootstrappedheap.ss")
...
the given element into the bootstrapped heap. Example
@schememod[
typed-scheme
(require "bootstrappedheap.ss")
...
The function @scheme[sorted-list] takes a bootstrapped heap and gives back a list 
