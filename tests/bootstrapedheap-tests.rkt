#lang typed-scheme
(require "../heap/bootstrapped/main.rkt")
...
(check-expect (sorted-list (insert -10 
                                   (heap less-than? 1 20 -3 -4)))
              (list -10 -4 -3 1 20))

(check-expect (sorted-list (insert 1 (heap less-than? 1 2 -3 -4)))
              (list -4 -3 1 1 2))

(check-expect (sorted-list (merge (ann (heap less-than? 2 3 -1 -10)
                                       (Heap Integer))
                                  (ann (heap less-than? 1 -2 -3 -4)
                                       (Heap Integer))))
              (list -10 -4 -3 -2 -1 1 2 3))

(check-expect (sorted-list 
               (merge (delete-min/max 
...
(check-expect (sorted-list (merge (ann (heap less-than? 1 -2 -3 -4)
                                       (Heap Integer))
                                  (delete-min/max 
                                    (ann (heap less-than? -1 2)
                                         (Heap Integer)))))
              (list -4 -3 -2 1 2))
